...
const SplashScreen = ({ onComplete }: SplashScreenProps) => {
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[hsl(var(--gov-dark))] transition-opacity duration-500 ${phase === "done" ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
      <div className={`transition-all duration-700 ease-out ${phase === "logo" ? "scale-50 opacity-0" : "scale-100 opacity-100"}`}>
        <img src={logoImg} alt="Logo" className="h-20 w-auto object-contain drop-shadow-2xl" />
      </div>
      ...
      <div className={`mt-10 w-48 transition-all duration-500 ${phase === "progress" || phase === "done" ? "opacity-100" : "opacity-0"}`}>
        <div className="h-1 w-full rounded-full bg-white/10 overflow-hidden">
          <div className="h-full rounded-full bg-gradient-to-r from-accent to-[hsl(var(--gov-gold))] transition-all duration-100 ease-linear" style={{ width: `${progress}%` }} />
        </div>
        <p className="mt-2 text-center text-[10px] text-white/30">Carregando...</p>
      </div>
      <div className={`absolute bottom-10 flex items-center gap-2 text-[10px] text-white/20 transition-all duration-500 ${phase !== "logo" ? "opacity-100" : "opacity-0"}`}>
