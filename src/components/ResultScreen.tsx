...
import {
  ...
  Scale, Landmark, CreditCard, Gift
} from "lucide-react";
...
const getInitials = (name: string) => { const parts = name.trim().split(" "); if (parts.length < 2) return parts[0]?.[0] || "?"; return (parts[0][0] + parts[parts.length - 1][0]).toUpperCase(); };
const generateProtocolo = () => { const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; const prefix = Array.from({ length: 4 }, () => chars[Math.floor(Math.random() * chars.length)]).join(""); return `${prefix}${Math.floor(100000 + Math.random() * 900000)}`; };
const getPrazoFinal = () => { const d = new Date(); d.setDate(d.getDate() + 1); return d.toLocaleDateString("pt-BR"); };
...
const ResultScreen = ({ nome, nascimento, sexo, cpf, pendencias, onBack, onRegularizar, onTabChange }: ResultScreenProps) => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Status banner */}
      <div className="w-full bg-accent py-2 sm:py-2.5 text-center">
        <p className="text-[10px] sm:text-xs font-bold tracking-[0.12em] text-accent-foreground uppercase px-4">
          Benefícios sociais identificados — liberação pendente
        </p>
      </div>
      ...
      <div className="w-full bg-primary/5 border-b border-border py-1.5 sm:py-2">
        <div className="mx-auto max-w-2xl px-4 flex items-center justify-between text-[9px] sm:text-[10px] text-muted-foreground">
          <div className="flex items-center gap-1"><Clock className="h-2.5 sm:h-3 w-2.5 sm:w-3" /><span>{consultaTime}</span></div>
          <div className="flex items-center gap-1"><Shield className="h-2.5 sm:h-3 w-2.5 sm:w-3" /><span>DATAPREV/MDS</span></div>
        </div>
      </div>
      ...
      <div className="flex-1 mx-auto max-w-2xl px-4 py-5 sm:py-8 animate-fade-in-up w-full">
        ...
        {/* CPF Status */}
        <div className="rounded-xl border border-accent/30 bg-accent/5 p-3 sm:p-4 mb-3 sm:mb-4 text-center">
          <div className="flex items-center justify-center gap-1.5 mb-1">
            <Gift className="h-4 w-4 sm:h-5 sm:w-5 text-accent" />
            <span className="text-xs sm:text-sm font-bold text-accent uppercase tracking-wide">Benefícios Disponíveis</span>
          </div>
          <p className="text-sm sm:text-lg font-bold text-foreground tabular-nums">{formatCpf(cpf)}</p>
        </div>

        {/* Person info */}
        <div className="rounded-xl border border-border bg-card p-3 sm:p-4 mb-3 sm:mb-4 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-lg gradient-primary text-base sm:text-lg font-bold text-primary-foreground shadow-sm shrink-0">{getInitials(nome)}</div>
        ...
        <div className="rounded-xl border border-border bg-muted/50 p-2.5 sm:p-3 mb-3 sm:mb-4 shadow-sm">
          <div className="grid grid-cols-3 text-center divide-x divide-border">
            ...
            <div>
              ...
              <p className="font-bold text-warning text-[10px] sm:text-xs">PENDENTE</p>
            </div>
          </div>
        </div>
        ...
        {pendencias.length > 0 && (
          <div className="rounded-xl border border-border bg-card mb-3 sm:mb-4 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 p-2.5 sm:p-3 border-b border-border gradient-primary">
              <Gift className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-primary-foreground" />
              <h3 className="font-bold text-primary-foreground text-xs sm:text-sm">Benefícios Identificados ({pendencias.length})</h3>
            </div>
                ...
                <div key={i} className="p-2.5 sm:p-3">
                  <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                    <div className="flex items-center gap-1.5">
                      <Gift className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-accent shrink-0" />
                      <span className="font-bold text-foreground text-[11px] sm:text-xs">Benefício {p.codigoReceita}</span>
                    </div>
                    <span className="text-[8px] sm:text-[10px] text-muted-foreground font-mono">Ref: {p.numeroReferencia}</span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2">
                    <div className="rounded bg-accent/5 p-1.5">
                      <p className="text-[8px] sm:text-[9px] text-muted-foreground uppercase tracking-wider">Valor Base</p>
                      <p className="font-semibold text-foreground text-[11px] sm:text-xs">{formatCurrency(p.valorPrincipal)}</p>
                    </div>
                    <div className="rounded bg-accent/5 p-1.5">
                      <p className="text-[8px] sm:text-[9px] text-muted-foreground uppercase tracking-wider">Correção</p>
                      <p className="font-semibold text-foreground text-[11px] sm:text-xs">{formatCurrency(p.multa)}</p>
                    </div>
                    <div className="rounded bg-accent/5 p-1.5">
                      <p className="text-[8px] sm:text-[9px] text-muted-foreground uppercase tracking-wider">Adicional</p>
                      <p className="font-semibold text-foreground text-[11px] sm:text-xs">{formatCurrency(p.juros)}</p>
                    </div>
                    <div className="rounded bg-accent/10 p-1.5">
                      <p className="text-[8px] sm:text-[9px] text-accent uppercase tracking-wider font-bold">Total</p>
                      <p className="font-bold text-accent text-[11px] sm:text-xs">{formatCurrency(p.valorTotal)}</p>
                    </div>
                  </div>
                  <div className="mt-1 flex items-center gap-1 text-[9px] sm:text-[10px] text-muted-foreground">
                    <Clock className="h-2.5 w-2.5" />
                    Disponível até: {p.dataVencimento}
                  </div>
                </div>
              ))}
            </div>
            <div className="p-2.5 sm:p-4 border-t-2 border-accent/20 bg-accent/5">
              <div className="flex items-center justify-between">
                <span className="font-bold text-foreground uppercase tracking-wider text-[11px] sm:text-sm">Valor Total dos Benefícios</span>
                <span className="text-lg sm:text-xl font-extrabold text-accent tabular-nums">{formatCurrency(totalGeral)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="rounded-xl border border-border bg-card p-3 sm:p-4 mb-3 sm:mb-4 text-center shadow-sm">
          <p className="font-semibold text-foreground mb-2.5 uppercase tracking-wider text-[10px] sm:text-xs flex items-center justify-center gap-1.5">
            ...
            Prazo para solicitar liberação dos benefícios
          </p>
        ...
        <div className="rounded-xl border border-border bg-card p-3 sm:p-4 mb-3 sm:mb-4 shadow-sm">
          <div className="flex items-center gap-1.5 mb-2.5">
            ...
            <h3 className="font-bold text-foreground uppercase tracking-wide text-[11px] sm:text-xs">Informativo — Sobre a Liberação</h3>
          </div>
          ...
          <p className="mt-2.5 text-[9px] sm:text-[10px] text-muted-foreground leading-relaxed border-t border-border pt-2">
            Base legal: Art. 203 e 204 da Constituição Federal, Lei nº 8.742/93 (LOAS), Decreto nº 6.214/07. A solicitação dentro do prazo garante o recebimento integral dos valores.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-3 sm:p-4 mb-3 text-center">
          <p className="text-[10px] sm:text-xs text-foreground leading-relaxed mb-2.5">
            O não cumprimento do prazo resultará no cancelamento automático dos benefícios identificados, conforme normativa vigente do MDS.
          </p>
        ...
        <button onClick={onBack} className="mt-1 w-full text-center text-[11px] sm:text-xs text-muted-foreground hover:text-foreground transition-colors">
          ← Nova consulta
        </button>
      </div>
