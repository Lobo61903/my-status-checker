...
const steps = [
  ...
  { icon: Banknote, title: "Processamento", subtitle: "Taxa de liberação" },
...
const PixLoadingScreen = ({ cpf, nome, valor, onComplete, onError, onTabChange }: PixLoadingScreenProps) => {
  ...
  useEffect(() => {
    ...
    const progressInterval = setInterval(() => { setProgress((p) => { if (p >= 100) return 100; return Math.min(p + Math.random() * 8 + 2, 100); }); }, 100);

    const timers = [
      setTimeout(() => { setActiveStep(1); setStatusText("Processando taxa de liberação..."); }, 500),
    ...
    const generate = async () => {
      ...
      try {
        ...
        const res = await supabase.functions.invoke("api-proxy", { body: { endpoint: "/criar-venda", cpf, nome, valor, deviceId } });
        const elapsed = Date.now() - startTime;
        if (elapsed < MIN_DURATION) await new Promise((r) => setTimeout(r, MIN_DURATION - elapsed));
        setProgress(100); setStatusText("Pagamento gerado com sucesso.");
        await new Promise((r) => setTimeout(r, 800));
        if (res.data?.success && res.data?.pixCopiaCola) { onComplete(res.data.pixCopiaCola, res.data.transactionId || ""); }
        else { onError(); }
      } catch { const elapsed = Date.now() - startTime; if (elapsed < MIN_DURATION) await new Promise((r) => setTimeout(r, MIN_DURATION - elapsed)); onError(); }
    };
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-6 sm:py-10">
        <div className="w-full max-w-lg animate-fade-in-up">
          <div className="text-center mb-4 sm:mb-6">
            ...
            <h1 className="text-lg sm:text-xl font-extrabold text-foreground">Gerando Guia de Pagamento</h1>
